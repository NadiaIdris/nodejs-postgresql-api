...
const deleteRegUserQuery = `DELETE FROM ${REG_USER} WHERE ${REG_USER_UID} = $1 RETURNING *`;
...
const getStudentsQuery = `SELECT * FROM ${STUDENT} ORDER BY CREATED_AT ASC`;
...
const updateStudentByUidQuery = `UPDATE ${STUDENT} SET ${STUDENT_FIRST_NAME} = $1, ${STUDENT_LAST_NAME} = $2, ${STUDENT_EMAIL} = $3, ${STUDENT_GENDER} = $4, ${STUDENT_DATE_OF_BIRTH} = $5, UPDATED_AT = NOW() WHERE ${STUDENT_UID} = $6 RETURNING *`;
